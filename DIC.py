...
def prep_fs():
    ...
    os.system("sudo cp isolinux.cfg ~/LIVE_BOOT/staging/isolinux/")
    
    os.system("sudo cp grub.cfg ~/LIVE_BOOT/staging/boot/grub")

    os.system("sudo cp grub-standalone.cfg /LIVE_BOOT/tmp")
    os.system("sudo touch $HOME/LIVE_BOOT/staging/DEBIAN_CUSTOM")

    #copy BIOS/legacy boot required files to workspace
    os.system("sudo cp /usr/lib/ISOLINUX/isolinux.bin \"~/LIVE_BOOT/staging/isolinux/\" && sudo cp /usr/lib/syslinux/modules/bios/* \"~/LIVE_BOOT/staging/isolinux/\"")
    #copy EFI boot files to ws
    os.system("sudo cp -r /usr/lib/grub/x86_64-efi/* \"~/LIVE_BOOT/staging/boot/grub/x86_64-efi/\"")

def gen_efi_img():
    os.system("sudo grub-mkstandalone --format=x86_64-efi --output=~/LIVE_BOOT/tmp/bootx64.efi --locales=\"\" --fonts=\"\" \"boot/grub/grub.cfg=~/LIVE_BOOT/tmp/grub-standalone.cfg\"")
